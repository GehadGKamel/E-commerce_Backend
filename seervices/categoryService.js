...
//@access Public
exports.getCategory = asyncHamdler( async (req, res) => {
    const { id } = req.params;
    const category = await Category.findById(id);
    if (!category) {
       res.status(404).json({ msg:`Category for this id ${id} not found`});
    }
...
//@access Private
exports.updateCategory = asyncHamdler( async (req, res) => {
    const { id } = req.params;
    const { name } = req.body;
...
//@access Private
exports.deleteCategory = asyncHamdler( async (req, res) => {
    const { id } = req.params;
